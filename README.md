...
### Search for Selected ORACC Text Intertextualities
The script also provides functionalities to run intertextuality detection for a wjole selected ORACC text. In this case, in the web UI, you can select the window length (number of tokens that is run as a query for intertextuality detection) and stride (an overlap of the windows). The mode for query search is done according to the setting set in the section "general setting" within UI.
